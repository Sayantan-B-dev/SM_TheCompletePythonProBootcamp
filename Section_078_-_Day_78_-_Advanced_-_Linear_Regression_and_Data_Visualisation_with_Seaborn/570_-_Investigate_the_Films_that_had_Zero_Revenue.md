...
After cleaning the dataset, the next logical step is to explore the data more deeply. This phase focuses on understanding the distribution of film budgets and revenues, identifying films that earned nothing (either domestically or worldwide), and examining the characteristics of those films. The analysis answers several key questions about the dataset, such as:

- What are the average production budget and worldwide gross revenue?
- How many films grossed $0 domestically or worldwide?
- Which high‑budget films failed to generate any revenue?
- Are there films that lost money (production costs exceeding revenue)?

The answers provide insight into the financial landscape of the film industry and help set the stage for more advanced visualisation and modelling.

All operations continue to use the pandas DataFrame `data`, which has been cleaned as described in the previous section.

---

## 1. Descriptive Statistics of the Dataset

We begin by generating summary statistics for the numeric columns using the `.describe()` method. This provides a quick overview of the central tendency, dispersion, and shape of the distribution for each variable.
...
- **Average production budget**: Approximately $31.1 million.
- **Average worldwide gross revenue**: Approximately $88.9 million, indicating that on average films earn about three times their production cost.
- **Minimum values**: The smallest budget is $1,100, and the minimum revenue (both domestic and worldwide) is $0. This suggests that some films either never screened or were complete flops.
- **Quartiles**: 
  - 25% of films have budgets ≤ $5 million and worldwide gross ≤ $3.9 million.
  - The median film costs $17 million and earns $27.5 million worldwide.
  - 75% of films have budgets ≤ $40 million and worldwide gross ≤ $96.5 million.
...
The next step is to investigate how many films earned nothing in the United States (domestic market). A domestic gross of $0 can occur for several reasons:

- The film was never released in the U.S.
- The film was released but earned no reported revenue (e.g., a complete failure).
- The film had not yet been released at the time of data collection.
...
To see which high‑budget films failed to earn anything in the U.S., we sort the subset by production budget in descending order:

```python
zero_domestic.sort_values('USD_Production_Budget', ascending=False)
```

The top rows of the sorted output:
